...
export function AreaMosaic({ items }: { items: readonly Area[] }) {
  return (
    <ul className="mt-12 grid gap-4 aparece-escalonado sm:grid-cols-2 lg:grid-cols-3">
      {items.map((area, i) => {
        ...
        return (
          <li key={area.slug} className={wide ? 'lg:col-span-2' : ''}>
            <Link
              ...
              className="group relative isolate flex h-full min-h-[17rem] flex-col items-center justify-end overflow-hidden bg-ink p-6 text-center text-white transition-[transform,box-shadow] duration-300 ease-plan hover:-translate-y-0.5 hover:shadow-[0_24px_48px_-28px_rgb(11_27_43/0.55)] focus-visible:-translate-y-0.5 active:translate-y-0 sm:p-7"
            >
              {area.image && (
                <SectionImage
                  name={area.image}
              ...
              <p className="mx-auto mt-2 max-w-prose text-step--1 leading-relaxed text-white/80">{area.blurb}</p>
            </Link>
          </li>
        );
      })}
