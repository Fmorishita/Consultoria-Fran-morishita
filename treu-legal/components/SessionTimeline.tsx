...
export function SessionTimeline({
  ...
  return (
    <ol className="mt-12 grid gap-y-10 lg:grid-cols-[var(--tramos)] lg:gap-x-8" style={{ ['--tramos' as string]: weights.map((w) => `${w}fr`).join(' ') }}>
      {steps.map((s, i) => (
        <li key={s.step} className="relative">
          ...
          <div className="border-t border-line pt-5 lg:border-t-0 lg:pt-6">
            ...
            <p className="mt-3 max-w-prose text-step--1 leading-relaxed text-slate">{s.text}</p>
          </div>
        </li>
      ))}
    </ol>
